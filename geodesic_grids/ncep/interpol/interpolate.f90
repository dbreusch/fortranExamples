...
! arrays for ncids of grid vars w/attributes we want to copy
        integer, parameter :: NSAVED_VARIDS=5
        integer, dimension(NSAVED_VARIDS) :: rmp_varids, out_varids

! save remap and input file names to use in get_dst_grid_info
        integer, parameter :: MAX_FILENAME=255
        character(len=MAX_FILENAME) :: remapfile
        character(len=MAX_FILENAME) :: datafile

        logical :: first, time_dim_exists
...
!-----------------------------------------------------------------------
!
  SUBROUTINE interp_init (rmpname, src_size, dst_size, dataname)
!
!-----------------------------------------------------------------------
! Purpose: 
...
!     remap_matrix
!   Arguments:
!     rmpname - name of the remapping file
...
!     dataame - name of the input data file
!
! Bugs:
...
!-----------------------------------------------------------------------
        ...
        character(len=*), intent(in) :: rmpname   
...
! set globally available remap file name to argument rmpname
        if (len_trim(rmpname) > MAX_FILENAME) then
           print *, "Warning: file name ", rmpname,                   &
                    " is longer than ", MAX_FILENAME, " characters."
        end if
        remapfile = rmpname

! set globally available input data file name to argument dataname
        if (len_trim(dataname) > MAX_FILENAME) then
           print *, "Warning: file name ", dataname,                   &
                    " is longer than ", MAX_FILENAME, " characters."
        end if
        datafile = dataname
...
!           call handle_error (status, outfilename)
           ...
           if (nDims .gt. 1) then
             ...
             call define_time_dim(outfileID)
           else
...
!
        integer :: ncid_data, varid, xtype, ndims, nAtts, i, alen
        integer :: attr_length
        integer, dimension(10) :: dimIds
        character(100) :: varnamein
        character(255) :: attname

!
! define var in output file
!
        if (time_dim_exists) then
          status = nf_def_var (outfileID, varname, NF_REAL,              &
                               dst_grid_rank+1, (/grid_dim_ids, time_dim_id/), xoutid)
        else
          status = nf_def_var (outfileID, varname, NF_REAL,              &
                               dst_grid_rank, grid_dim_ids, xoutid)
        end if
        if (status /= nf_noerr) call handle_error (status, varname)

!
! copy var attributes from input to output
!
        status = nf_open( datafile, nf_nowrite, ncid_data )
        if (status /= nf_noerr) call handle_error (status, "open datafile")

        status = nf_inq_varid( ncid_data, varname, varid )
        if (status /= nf_noerr) call handle_error (status, "get var id")

        status = nf_inq_var( ncid_data, varid, varnamein, xtype, ndims, dimIds, nAtts )
        if (status /= nf_noerr) call handle_error (status, "get var info")

        do i=1,nAtts
          status = nf_inq_attname( ncid_data, varid, i, attname )
          if (status /= nf_noerr) call handle_error (status, "get attribute info")
          alen = len_trim( attname )
          status = nf_copy_att( ncid_data, varid, attname(1:alen), outfileID, xoutid )
          if (status /= nf_noerr) call handle_error (status, "copy attribute")
        end do

        status = nf_close( ncid_data )
        if (status /= nf_noerr) call handle_error( status, "Close input failed")
...
!-----------------------------------------------------------------------
        ...
        integer :: fileID, varID, status, nPoints, nTimes
        integer :: i, j, k

! get dimensions of var to be written out
        nTimes = size( xout, 1 )
        nPoints = size( xout, 2 )
        write(6, 1050) nTimes, nPoints
  1050  format('  Output file: ', i3, ' Time(s),', i6, ' Points')
        allocate( temp_data( nTimes*nPoints ))

! convert array to a vector
        do i=1,nTimes
          j = nPoints * (i-1) + 1
          k = nPoints * i
          temp_data(j:k) = xout(i,:)
        end do

! write the data

!        status = nf_put_var_real (outfileID, xoutid, xout)
        status = nf_put_var_real (outfileID, xoutid, temp_data)
...
!        status = nf_put_var_double (outfileID, xoutid, xout)
        status = nf_put_var_double (outfileID, xoutid, temp_data)
        call handle_error (status, varname)
...
!-----------------------------------------------------------------------
!
        SUBROUTINE define_time_dim(fileID)
!
!-----------------------------------------------------------------------
!  Purpose:
!    Define time dimension in output file
!
!  Scope: private
!    Module variables used:
!       
...
!-----------------------------------------------------------------------
        ...
        integer, intent(in) :: fileID
        integer :: status, ncid_data
        integer :: time_size, time_var_id, time_var_id_in
        real*8, dimension(:), allocatable :: time_array

        integer :: xtype, ndims, nAtts, i, alen
        integer :: attr_length
        integer, dimension(10) :: dimIds
        character(100) :: varnamein
        character(255) :: attname
...
! open input file
        status = nf_open (datafile, NF_NOWRITE, ncid_data)
        call handle_error (status, "file = "//datafile)

! get time dimension info from input file
        status = nf_inq_dimid (ncid_data, "time", time_dim_id)
        call handle_error (status, "time axis not found")
        status = nf_inq_dimlen (ncid_data, time_dim_id, time_size)
        call handle_error (status, "time_size")

! get time from input file
        allocate (time_array(time_size))
        status = nf_inq_varid (ncid_data, "time", time_var_id)
        call handle_error (status, "inq time var")
        status = nf_get_var_real (ncid_data, time_var_id, time_array)
        call handle_error (status, "get time var")

! define necessary dimensions in output file
        status = nf_def_dim(fileID, "time", time_size, time_dim_id)
        call handle_error (status, "time dim def")

! define time var in output file
        status = nf_def_var(fileID, "time", NF_DOUBLE, 1, time_dim_id, time_var_id)
        call handle_error (status, "write time dim")

!
! copy time attributes from input to output
!
        status = nf_inq_varid( ncid_data, "time", time_var_id_in )
        if (status /= nf_noerr) call handle_error (status, "get time in id")

        status = nf_inq_var( ncid_data, time_var_id_in, varnamein, xtype, ndims, dimIds, nAtts )
        if (status /= nf_noerr) call handle_error (status, "get time in info")

        do i=1,nAtts
          status = nf_inq_attname( ncid_data, time_var_id_in, i, attname )
          if (status /= nf_noerr) call handle_error (status, "get attribute info")
          alen = len_trim( attname )
          status = nf_copy_att( ncid_data, time_var_id_in, attname(1:alen), fileID, time_var_id )
          if (status /= nf_noerr) call handle_error (status, "copy attribute")
        end do

! write time var to output file
        status = nf_enddef(fileID)
        call handle_error (status, "end def")
        status = nf_put_var_real (fileID, time_var_id, time_array)
        call handle_error (status, "write time var")
        status = nf_redef (fileID)
        call handle_error (status, "redef")

        deallocate (time_array)
        status = nf_close (ncid_data)
        call handle_error (status)

        END SUBROUTINE define_time_dim
...
!-----------------------------------------------------------------------
        ...
        integer :: xtype, ndims, nAtts, alen
        integer :: attr_length
        integer, dimension(10) :: dimIds
        character(100) :: varnamein
        character(255) :: attname
...
!
! copy selected dimension attributes from the remap file to the output file
!

! re-open remapping file
        status = nf_open (remapfile, NF_NOWRITE, rmpfileID)
        call handle_error (status, "file = "//remapfile)

! switch data file to define mode
        status = nf_redef (fileID)
        call handle_error (status, "redef output file")

        do i=1,NSAVED_VARIDS
          status = nf_inq_var(rmpfileID, rmp_varids(i), varnamein, xtype, ndims, dimIds, nAtts)
          if (status /= nf_noerr) call handle_error (status, "get remap var nAtts")
          do j=1,nAtts
            status = nf_inq_attname( rmpFileID, rmp_varids(i), j, attname )
            if (status /= nf_noerr) call handle_error (status, "get remap var attribute info")
            alen = len_trim( attname )
            status = nf_copy_att(rmpFileID, rmp_varids(i), attname(1:alen), fileID, out_varids(i) )
            if (status /= nf_noerr) call handle_error (status, "copy remap attribute")
          end do
        end do

! switch data file to data mode
        status = nf_enddef(fileID)
        call handle_error (status, "enddef output file")

! done with remap file 
        status = nf_close (rmpfileID)
        call handle_error (status)
